...
// headless:false — the GLB + draco decode needs a real GPU compositor (per repo notes).
...
await page.waitForTimeout(9000); // GLB download + draco decode + first render
...
for (const f of fracs) {
  ...
  await page.screenshot({ path: `scripts/shots/coaster_${Math.round(f * 100)}.png`, timeout: 60000 });
}
